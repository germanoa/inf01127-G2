...
class IP(models.Model):
    ...
    def __dotted_to_intIP(dottedip):
    ...
    def set_address(self,dottedIP,version):
        if version==4:
            self.address = __dotted_to_intIP(dottedIP)

    def get_address(self):
        return __int_to_dottedIP(self.address)

    def set_mask(self,dottedMASK,version):
        if version==4:
            self.mask = __dotted_to_intIP(dottedMASK)

    def get_mask(self):
        return __int_to_dottedIP(self.mask)
    ...
    def get_net(self,t):
        ...
        mask= self.get_mask()
        if t=="int":
            return __ip_to_net(ip,mask)
        elif t=="dotted":
            return __int_to_dotted(__ip_to_net(ip,mask))
        else:
            return -1
